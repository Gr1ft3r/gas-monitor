...
fetchPrices();
setAlertModal({ isOpen: true, title: "Report Received", message: "Report received!", isError: false });
  }
...
if (votes >= 3) {
  // ... database updates
  setAlertModal({ isOpen: true, title: "Fuel Removed", message: "Fuel removed! 3 users have confirmed it.", isError: false });
} else {
  // ... database updates
  setAlertModal({ isOpen: true, title: "Report Received", message: `Report received! (${votes}/3)`, isError: false });
}
fetchPrices();
  }
...
async function handleAddStation(e) {
  ...
  if (existingStation && existingStation.length > 0) {
    ...
    if (existingFuel && existingFuel.length > 0) {
      ...
      setAlertModal({ isOpen: true, title: "Update Submitted", message: `Success! Updated existing ${fuelType} at ${fullStationName}. Awaiting community verification.`, isError: false });
    } else {
      ...
      setAlertModal({ isOpen: true, title: "New Fuel Added", message: `Success! Added new ${fuelType} to ${fullStationName}. Awaiting verification.`, isError: false });
    }
  } else {
    ...
    if (newStation && newStation.length > 0) {
      ...
      setAlertModal({ isOpen: true, title: "Station Added", message: `Thank you! ${fullStationName} has been added to the map as an Unverified Location. It will be fully verified once 2 more drivers confirm it.`, isError: false });
    }
  }
...
if (rawPrices.length > 0) {
  ...
  latestUpdate = maxDate.toLocaleString('en-PH', { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
}
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-sm p-6 border border-gray-200 text-center">
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg font-bold text-white transition-colors ${alertModal.isError ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
              Got it
            </button>
          </div>
