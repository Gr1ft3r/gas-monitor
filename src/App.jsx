...
function App() {
  ...
  async function handleUpvotePrice(priceId, currentUpvotes, stationId, fuelType) {
    ...
    if (logError && logError.code === '23505') { alert("Anti-Spam: You have already verified this price!"); return; }
  ...
  async function handleRetiredFuel(priceId, currentVotes) {
    ...
    if (logError && logError.code === '23505') { alert("Anti-Spam: You already reported this fuel as not sold!"); return; }
    ...
    if (votes >= 3) {
      ...
      alert("Fuel removed! 3 users have confirmed it.");
    } else {
      await supabase.from('prices').update({ retired_votes: votes }).eq('id', priceId);
      alert(`Report received! (${votes}/3)`);
    }
    fetchPrices();
  }
